...
export class CountriesSeeder {
  ...
  async seed(): Promise<void> {
    this.logger.log('Starting countries seed...');

    const countriesData = this.getCountriesData();

    this.logger.log(`Preparing to seed ${countriesData.length} countries...`);
    ...
    for (const countryData of countriesData) {
      try {
        const existing = await this.db.prisma.country.findUnique({
          where: { iso2Code: countryData.iso2Code },
        });

        if (existing) {
          await this.db.prisma.country.update({
            where: { iso2Code: countryData.iso2Code },
            data: countryData,
          });
          updated++;
        ...
        this.logger.warn(
          `Failed to seed country ${countryData.countryName}: ${error.message}`,
        );
        skipped++;
      }
    }
    ...
    this.logger.log(
      `Countries seed completed: ${created} created, ${updated} updated, ${skipped} skipped`,
    );
  }
